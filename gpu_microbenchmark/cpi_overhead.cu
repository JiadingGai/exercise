// cpi_ldx_micro_pure_lds.cu
// Micro-only CPI benchmark for LDG/LDS. Adds:
// 1) LDS mask-wrap addressing (no compare/select wrap) for lower overhead.
// 2) Overhead-subtracted "pure LDS" CPI by timing an overhead-only loop
//    that matches index + FP-add work (no loads), then subtracting.
...
__device__ __forceinline__ int pow2_floor(int x) {
  // return largest power of two <= x (assumes x > 0)
  x |= x >> 1; x |= x >> 2; x |= x >> 4; x |= x >> 8; x |= x >> 16;
  return (x + 1) >> 1;
}

// ---- unrolled bodies ----
// LDG: unchanged ring (index in floats, wrap via compare). This path is left as-is.
template<int UNROLL>
__device__ __forceinline__
...
// LDS: mask-based wrap (index in BYTES). Removes compare/select wrap.
template<int UNROLL>
__device__ __forceinline__
...
#pragma unroll
  for (int u=0; u<UNROLL; ++u) {
    ...
    a0+=x0; a1+=x1; a2+=x2; a3+=x3;          // 4 FADD per 128b load
    ofs_bytes = (ofs_bytes + 16) & mask_bytes; // wrap via AND, no branches
  }
}

// Overhead-only body (no LDS). Matches index update + 4 FADD per step.
template<int UNROLL>
__device__ __forceinline__
...
#pragma unroll
  for (int u=0; u<UNROLL; ++u) {
    // 4 FP adds (same count as do_unrolled_lds_mask), via inline PTX to avoid folding
...
// LDS with mask + overhead-subtracted timing
template<int UNROLL>
                                 ...
                                 int iters, int smem_floats)
{
  ...
  const int ring_bytes  = pow2_floor(smem_bytes);           // <= allocated bytes
  const int mask_bytes  = ring_bytes - 16;                  // ring in [0..N-16]
  int ofs0 = ((lane * 16) & mask_bytes);                    // byte offset, 16B aligned
...
#pragma unroll 1
  for (int i=0;i<iters;++i){
    do_overhead_mask<UNROLL>(smem, ofs_ovh, mask_bytes, a0,a1,a2,a3);
  }
  __syncthreads();
  ...
  int ofs_ld = ofs0;
  __syncthreads();
  unsigned long long t2 = rdclk();
#pragma unroll 1
  for (int i=0;i<iters;++i){
    do_unrolled_lds_mask<UNROLL>(smem, ofs_ld, mask_bytes, b0,b1,b2,b3);
  }
  __syncthreads();
  unsigned long long t3 = rdclk();
...
static void launch_micro_ldg(dim3 grid, dim3 block,
                             ...
                             int iters, int wss_elems, int use_cg)
{
  micro_ldg_kernel<UNROLL><<<grid, block>>>(d_cycles_with, d_sink, d_gbuf, iters, wss_elems, use_cg);
}
...
static void launch_micro_lds(dim3 grid, dim3 block, size_t shmem,
                             ...
                             int iters, int smem_floats)
{
  micro_lds_kernel<UNROLL><<<grid, block, shmem>>>(d_cycles_with, d_cycles_ovh, d_sink, iters, smem_floats);
}
...
int main(int argc, char** argv) {
  ...
  // LDG buffer (always allocate; cheap and avoids branching on alloc)
  ...
  auto run_one = [&](const char* op, const char* cache, int unroll)->void{
    // warmup for LDG to prime caches
    if (eq(op,"ldg")) {
    ...
    if (eq(op,"ldg")) {
      ...
      dispatch_lds_unroll(unroll, grid, block, shmem, d_cycles_with, d_cycles_ovh, d_sink,
                          a.iters, a.smem_f);
    }
    CHECK_CUDA(cudaDeviceSynchronize());

    // readback + print
    uint64_t cyc_with=0, cyc_ovh=0;
    CHECK_CUDA(cudaMemcpy(&cyc_with, d_cycles_with, sizeof(uint64_t), cudaMemcpyDeviceToHost));
    if (eq(op,"lds")) CHECK_CUDA(cudaMemcpy(&cyc_ovh,  d_cycles_ovh,  sizeof(uint64_t), cudaMemcpyDeviceToHost));

    const double insts = double(a.iters) * double(unroll) * 32.0;    // per-thread 128b ops
    const double cpi_t = double(cyc_with) / insts;                    // per-thread CPI
    const double cpi_w = cpi_t * 32.0;                                // warp-level CPI

    if (eq(op,"lds")) {
      const double cpi_pure_t = double(cyc_with - cyc_ovh) / insts;   // subtract overhead
      ...
      // LDG: no pure columns; print dashes to keep alignment
      printf("%-4s %-2s %6d %9d %14llu %14.0f %14.8f %14.8f %14s %14s\n",
             op, eq(cache,"cg")?"cg":"ca", unroll, a.iters,
             (unsigned long long)cyc_with, insts,
             cpi_t, cpi_w, "--", "--");
    }
  };
  ...
  if (a.sweep) {
    ...
    printf("%-4s %-2s %6s %9s %14s %14s %14s %14s %14s %14s\n",
           "----","--","------","---------","--------------","--------------","--------------","--------------","--------------","--------------");
    // LDS across all UNROLLS (with pure CPI)
    for (int u: UNROLLS) run_one("lds","--",u);
    // LDG across cache modes and all UNROLLS (no pure CPI)
